...
export const ScoreDisplay = ({ gameState, showParanoia = true }: ScoreDisplayProps) => {
  ...
  const displayedPopulation = getDistortedScore(scores.population, scores.paranoia);
  const displayedGovernment = getDistortedScore(scores.government, scores.paranoia);
    ...
    <div className="w-full max-w-4xl mx-auto">
          ...
          <div className="p-4 border border-paranoia/30 bg-paranoia/5 rounded-lg">
            ...
            <div className="h-2 bg-muted rounded-full overflow-hidden">
              <div 
                ...
                style={{ width: `${Math.min(100, (scores.paranoia / 50) * 100)}%` }}
              />
            </div>
            <p className="text-xs text-muted-foreground mt-1 font-mono">
      ...
      {gameState.consequences.length > 0 && (
        <div className="mt-4 p-4 border border-destructive/30 bg-destructive/5 rounded-lg">
          <h4 className="font-mono font-bold text-destructive mb-2">CONSEQUÊNCIAS ATIVAS:</h4>
