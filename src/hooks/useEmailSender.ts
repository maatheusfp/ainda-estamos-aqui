...
export const useEmailSender = () => {
  ...
  const sendSessionDataByEmail = async (
    ...
    try {
      ...
      const fileName = `sessao_jogo_${sessionData.playerData.age}anos_${
  ...
  const formatSessionDataAsText = (sessionData: SessionData): string => {
    ...
    text += "-".repeat(30) + "\n";
    text += `Início: ${new Date(sessionInfo.startTime).toLocaleString(
      "pt-BR"
    )}\n`;
    ...
    text += `Tempo Médio por Decisão: ${Math.round(
